...
void main() {
    // Get normalized [0,1] components
    vec2 yComponent = texture2D(yTex, TexCoord).ra;
    vec2 uComponent = texture2D(uTex, TexCoord).ra;
    vec2 vComponent = texture2D(vTex, TexCoord).ra;

    // Convert back to 0-255 range and reconstruct 10-bit value
    float y = (yComponent.r * 255.0 + yComponent.g * 255.0 * 256.0) / 1023.0;
    float u = (uComponent.r * 255.0 + uComponent.g * 255.0 * 256.0) / 1023.0;
    float v = (vComponent.r * 255.0 + vComponent.g * 255.0 * 256.0) / 1023.0;
...
// Structure to hold shader program and locations
    ...
    void convert_frame(YUVConverter* converter, AVFrame* frame) {
        ...
        glBindTexture(GL_TEXTURE_2D, converter->vTex);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frame->width/2, frame->height/2,
                        GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, frame->data[2]);
...
// Initialize shaders and textures
    ...
    void YUVHelper10::init_shader_and_textures(int width, int height) {
        yuvConverter = create_yuv_converter(width, height);
        rgbaTexture = ((YUVConverter*)yuvConverter)->rgbaTex;
    }

// Function to update textures with new frame data
    void YUVHelper10::update_textures(AVFrame *frame) {
        // Update Y plane
        convert_frame((YUVConverter*)yuvConverter, frame);
    }
...
// Main usage example
    void YUVHelper10::process_frame(AVFrame *frame) {
        if (!initialized) {
