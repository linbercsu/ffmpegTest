...
//#define bh_log(format, ...)

#define mc_log(format, ...) __android_log_print(6, "MediaConverter2", format, __VA_ARGS__)
...
namespace {
    ...
    class MediaConverter : public ProcessCallback {

    public:
        static void initClass(JNIEnv *pEnv, jclass clazz);

        MediaConverter(JNIEnv *env,
                       jobject thzz, const char *sourcePath, const char *targetPath,
                       const char *format);

        ~MediaConverter() noexcept override;

        void onProgress(int progress) override;

        const char *convert(int64_t start, int64_t duration, int audioIndex, int videoIndex);
    ...
    const char MediaConverter::TAG[] = "MediaConverter";
    ...
    public:
        ...
        virtual void onDurationGot(int64_t duration) = 0;
    ...
    public:
        explicit InputStream(InputStreamCallback *callback,
                             const char *path):
...
//            mc_log("on audio frame %d", __LINE__);
            callback->onAudioFrame(audioFrame);
        }

        void output_video_frame(AVFrame *pFrame) {
//            mc_log("on video frame %ld", pFrame->pts);
            callback->onVideoFrame(pFrame);
        }

        int decode_packet_audio(AVCodecContext *dec, const AVPacket *package) {
            int ret;
            ...
            while (ret >= 0) {
                ret = avcodec_receive_frame(dec, mAudioFrame);
                if (ret < 0) {
...
//                return ret;
                ...
                output_audio_frame(mAudioFrame);
...
//                return ret;
        ...
        static int find_stream_by_index(AVFormatContext *formatContext, enum AVMediaType type,
            ...
            if (index_audio_track < streams) {
                for (int i = 0; i < streams; ++i) {
                    ...
                    __android_log_print(6, "AudioConverter",
                                        "i:%d  codec_type:%d index_audio_track:%d", i, codec_type,
                                        index_audio_track);
                    if (codec_type == type && i == index_audio_track) {
                        ret = i;
                    }
                }
            }
        ...
        static void open_codec_context(int *stream_idx,
                                       ...
                                       enum AVMediaType type, int index) {
            ...
            if (index == -1) {
                ...
                ret = find_stream_by_index(formatContext, type, index);
                if (ret < 0)
                    return;
            }
...
//                return ret;
    ...
    public:
        ...
        void init(int64_t pStart, int audioIndex, int videoIndex) {
            ...
            open_codec_context(&audio_stream_idx, &audio_dec_ctx, fmt_ctx, AVMEDIA_TYPE_AUDIO, audioIndex);
            ...
            int64_t audioDuration = -1, videoDuration = -1;
            if (audio_stream_idx != -1) {
                audio_stream = fmt_ctx->streams[audio_stream_idx];

                if (audio_stream->duration > 0) {
                    audioDuration = av_rescale_q(audio_stream->duration,
                                                 audio_stream->time_base,
                                                 AV_TIME_BASE_Q);
                }
                mc_log( "find audio stream %d", audio_stream->codecpar->codec_id);
            ...
            open_codec_context(&video_stream_idx, &video_dec_ctx, fmt_ctx, AVMEDIA_TYPE_VIDEO, videoIndex);
            if (video_stream_idx != -1) {
                ...
                if (video_stream->duration > 0) {
                    videoDuration = av_rescale_q(video_stream->duration,
                ...
                mc_log("find video stream %d %d", pix_fmt, video_stream->codecpar->codec_id);
            ...
            if (video_stream_idx != -1) {
                int64_t videoStart = av_rescale_q(pStart,
                                                  AV_TIME_BASE_Q,
                                                  video_stream->time_base);
                av_seek_frame(fmt_ctx, video_stream_idx, videoStart, AVSEEK_FLAG_BACKWARD);
            } else {
                int64_t audioStart = av_rescale_q(pStart,
                             AV_TIME_BASE_Q,
                             audio_stream->time_base);
                av_seek_frame(fmt_ctx, audio_stream_idx, audioStart, AVSEEK_FLAG_BACKWARD);
            }

            int64_t maxDuration = videoDuration;
            if (videoDuration < audioDuration) {
                maxDuration = audioDuration;
            }

            if (maxDuration < 0) {
                maxDuration = fmt_ctx->duration;
            }

            callback->onDurationGot(maxDuration);
        }
        ...
        void start() {
            ...
            while (!isStopped() && av_read_frame(fmt_ctx, pkt) >= 0) {
                if (pkt->stream_index == audio_stream_idx) {
                    ret = decode_packet_audio(audio_dec_ctx, pkt);
            ...
            if (audio_dec_ctx)
                decode_packet_audio(audio_dec_ctx, nullptr);
...
//                throw ConvertException("buffer internal error");
    ...
    public:
        OutputStream(ProcessCallback *processCallback, const char *targetPath, const char *format)
                : processCallback(processCallback), targetPath(targetPath), format(format) {
...
//        __android_log_write(6, "AudioConverter", "~OutputStream");
    ...
    private:
        ...
        int64_t needMaxDuration{-1};
        int64_t gotDuration{-1};
        ...
        int64_t lastAudioPacketPts{-1};
        int64_t firstAudioPacketPts{-1};
        bool firstAudioPacketPtsGot{false};
        int64_t lastVideoPacketPts{-1};
        int64_t firstVideoPacketPts{-1};
        bool firstVideoPacketPtsGot{false};
        int64_t lastCallbackPts{0};
    public:
        void setMaxDuration(int64_t d) {
            this->needMaxDuration = d;
        }
...
//                __android_log_print(6, "AudioConverter", "convert end");
                converted->pts = av_rescale_q(p - firstVideoPts, sourceVideoTimebase, c->time_base);
...
//            __android_log_print(6, "AudioConverter", "ffmpeg convert");
        ...
        AVFrame *rotateVideo(AVFrame *pFrame) {
            ...
            if (pFrame->format == AV_PIX_FMT_YUV420P) {
                if (videoFrameRotate == nullptr)
                    videoFrameRotate = alloc_picture(AV_PIX_FMT_YUV420P, videoCodecContext->width,
        ...
        AVFrame *convertVideo(AVFrame *pFrame) {
            ...
            if (videoFrameConvert == nullptr) {
                videoFrameConvert = alloc_picture(AV_PIX_FMT_NV12, videoCodecContext->width,
                                                  videoCodecContext->height);
            }
...
//        log_packet(fmt_ctx, &pkt);
        ...
        void onWriteAudio(AVPacket* packet) {
            if (packet->pts == AV_NOPTS_VALUE)
                return;

            lastAudioPacketPts = av_rescale_q(packet->pts,
                                              stream->time_base,
                                              AV_TIME_BASE_Q);

            if (!firstAudioPacketPtsGot) {
                firstAudioPacketPtsGot = true;
                firstAudioPacketPts = lastAudioPacketPts;
            }

            mc_log("onWriteAudio %ld, %ld, %ld", lastAudioPacketPts, packet->pts, packet->dts);

            auto d = knownDuration();
            if (d < 0)
                return;

            if (lastAudioPacketPts > 0) {
                auto elapse = lastAudioPacketPts - lastCallbackPts;
                if (elapse > 500000) {
                    lastCallbackPts = lastAudioPacketPts;
                    auto p = lastCallbackPts * 100 / d;
                    processCallback->onProgress(p);
                }
            }
        }
        void onWriteVideo(AVPacket* packet) {
            if (packet->pts == AV_NOPTS_VALUE)
                return;

            lastVideoPacketPts = av_rescale_q(packet->pts,
                                              videoStream->time_base,
                                              AV_TIME_BASE_Q);

            if (!firstVideoPacketPtsGot) {
                firstVideoPacketPtsGot = true;
                firstVideoPacketPts = lastVideoPacketPts;
            }

            mc_log("onWriteVideo %ld, %ld, %ld", lastVideoPacketPts, packet->pts, packet->dts);

            auto d = knownDuration();
            if (d < 0)
                return;

            if (lastVideoPacketPts > 0) {
                auto elapse = lastVideoPacketPts - lastCallbackPts;
                if (elapse > 500000) {
                    lastCallbackPts = lastVideoPacketPts;
                    auto p = lastCallbackPts * 100 / d;
                    processCallback->onProgress(p);
                }
            }
        ...
        int64_t knownDuration() const {
            if (needMaxDuration > 0) {
                return needMaxDuration;
            }
        ...
        int64_t currentAudioDuration() const {
            if (lastAudioPacketPts > 0)
                return lastAudioPacketPts - firstAudioPacketPts;

            return 0;
        }

        int64_t currentVideoDuration() const {
            if (lastVideoPacketPts > 0)
                return lastVideoPacketPts - firstVideoPacketPts;
        ...
        void onDurationGot(int64_t pDuration) override {
            this->gotDuration = pDuration;
        }
        ...
        void onAudioStream(AVCodecContext *sourceCodecContext) override {
            ...
            mc_log( "onAudioStream %d, %d", sourceSample_rate,
                                sourceSampleFormat);
        ...
        void onVideoStream(AVCodecContext *sourceCodecContext, AVStream *st) override {
            ...
            mc_log("onVideoStream %d, %d, %d, %d", sourceWidth,
...
//            write_audio_frame(audioFrame);
        ...
        bool enough() override {
            ...
            if (currentVideoDuration() > needMaxDuration || currentAudioDuration() > needMaxDuration) {
                mc_log("enough %ld, %ld %ld", currentVideoDuration(), currentAudioDuration(), needMaxDuration);
                return true;
            }
    ...
    MediaConverter::MediaConverter(JNIEnv *env,
                                   jobject thzz, const char *sourcePath,
                                   const char *targetPath, const char *format)
            :callback(new JavaProgressCallback(env, thzz)),
            target(new OutputStream(this, targetPath, format)),
              inputStream(new InputStream(target, sourcePath)) {
    }

    MediaConverter::~MediaConverter() noexcept {
        if (callback == nullptr)
            return;
    ...
    void MediaConverter::onProgress(int progress) {
        callback->onProgress(progress);
    }

    const char *MediaConverter::convert(int64_t start, int64_t duration, int audioIndex, int videoIndex) {
        try {
            if (duration > 0) {
                target->setMaxDuration(duration);
            }

            inputStream->init(start, audioIndex, videoIndex);
            inputStream->start();
//        inputStream.reset();
//        target.reset();
        } catch (std::exception &e) {
//        __android_log_write(6, "MediaConverter", e.what());
            return e.what();
        }
...
//////////////jni
    ...
    jlong nativeInit(JNIEnv *env,
        ...
        auto *converter = new MediaConverter(env, thzz, sourcePath, targetPath, formatUTF);
    ...
    jstring nativeConvert(JNIEnv *env,
                                 jobject  /*thzz*/, jlong ptr, jlong startPosition, jlong duration, jint audioIndex, jint videoIndex) {
        auto *converter = reinterpret_cast<MediaConverter *>(ptr);
        const char *error = converter->convert(startPosition, duration, audioIndex, videoIndex);
        if (error == nullptr)
            return nullptr;
    ...
    const JNINativeMethod methods[] =
            {
                    ...
                    {"nativeConvert", "(JJJII)Ljava/lang/String;",                                     (void *) nativeConvert},
...
//    }
    ...
    Java_com_mxtech_av_MediaConverter2_nativeInitClass(
            JNIEnv* env,
            jclass clazz) {
